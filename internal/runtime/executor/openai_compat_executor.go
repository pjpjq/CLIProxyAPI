...
func (e *OpenAICompatExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func() {
		...
		processFrame := func() bool {
			...
			if nativeResponses && !isDone {
				if streamErr, _, ok := codexTerminalFailureErr(dataPayload); ok {
			...
			if nativeResponses {
				switch gjson.GetBytes(dataPayload, "type").String() {
		...
		publishNativeMetadata := func(line []byte) bool {
			if !nativeResponses {
				return false
			}
	...
	scanLoop:
		...
		if errScan != nil {
			...
			return
		} else if !nativeResponses && !seenDone {
...
func (e statusErr) IsCredentialAvailabilityNeutral() bool {
	return e.availabilityNeutral
}
